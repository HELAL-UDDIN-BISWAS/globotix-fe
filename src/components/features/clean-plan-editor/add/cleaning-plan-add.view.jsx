...
const CleaningPlanAddView = ({ planDetail, setPlanDetail }) => {
  ...
  const submit = async (data) => {
    ...
    router.push(`/cleaning-plan/add?mode=zone&from=${from}`);
  };
  ...
  useEffect(() => {
    if (location?.data?.length > 0) {
      const buildingFilteredLocations = location?.data.filter(
        (item) => item.building.id == selectedBuilding?.id
      );
      const filteredLocations = buildingFilteredLocations.filter(
        (item) => item.zone.length > 0
      );

      let arr = filteredLocations?.map((item) => {
        return {
          ...item,
          value: item.id,
          label: item?.name,
  ...
  }, [location?.data, selectedBuilding]);
