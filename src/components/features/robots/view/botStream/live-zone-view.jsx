...
import ZoneLiveViewPage from "./zone-live-view";
...
const LiveZoneView = ({
  ...
  const [selectedDate, setSelectedDate] = useState(
    new Date("2024.08.15 1:40 PM")
  );
  const [chosenDate, setChosenDate] = useState(new Date("2024.08.15 1:40 PM"));
  const [hours, setHours] = useState(new Date().getHours() - 1);
  const [minutes, setMinutes] = useState(new Date().getMinutes() - 10);
  ...
  const [startTime, setStartTime] = useState(
    dayjs("2024.08.15 1:40 PM").subtract(10, "minute").valueOf() * 1000000
  );
  const [endTime, setEndTime] = useState(
    dayjs("2024.08.15 1:40 PM").valueOf() * 1000000 // Convert to nanoseconds
  );
      ...
      await axios
        ...
        .then((response) => {
          setPathData((prevPathData) => {
            ...
            return prepareEntries.length > 0
